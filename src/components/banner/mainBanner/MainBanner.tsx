...
const MainBanner = () => {
  const [isActives, setIsActive] = useState<boolean[]>([
    true,
    false,
    false,
    false,
  ...
  const handleToggle = (e: number) =>
    setIsActive(isActives.map((prev, idx) => (e === idx ? true : false)));
      ...
      <article className={styles.article}>
        ...
        <div className={styles.accordion}>
          {menuList.map((menu, idx: number) => {
            return (
              <div className={styles.accordionCont} key={idx}>
                <div
                  className={
                    isActives[idx]
